...
- **Visual Simulation Mode (What-If)**: Driven directly by the configuration-level ATM Ratio Entry settings (`atmRatioScaling` toggle and `atmRatioDistanceCall` / `atmRatioDistancePut` offsets). Toggling ATM Ratio Entry ON automatically simulates quantities, margins, net premiums, and projected PnLs under the 200X leverage limit ($200k portfolio cap) in the scanner, highlighting modified candidate ratios in golden text. The manual visual Base/Extra credit toggle has been completely removed from both scanner and paper trading screens; Paper Trading renders database metrics as-is.
