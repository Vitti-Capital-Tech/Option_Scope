...
- **Paper Trading**: Fully automated strategy lifecycle simulation — entry at Ask/Bid, 1s-cadence live PnL based on liquidation value, full ATM exits, standard portfolio rotation, expiry settlement, IV tracking, and trade history export.
...
- **Strict Execution-Realistic Paper Trading**: High-fidelity simulation using Ask prices for long legs and Bid prices for short legs. New entries are strictly rejected if real-time, executable quotes (Ask/Bid) are missing from the order book, or if they are stale. The system distinguishes between live WebSocket updates and stale/model-derived REST backfill prices by timestamping incoming quotes. Only WebSocket-confirmed quotes updated within the last 120 seconds are eligible for entry execution, preventing false fills on illiquid strikes.
...
- **ATM Ratio & Price Tracking**: Captures the exact ATM option prices and computed ratio at the moment of entry and exit (full/partial). Displays these values as **Entry ATM Ratio (Prices)** and **Exit ATM Ratio (Prices)** columns in the Trade History UI table and exports them to CSV for advanced analysis.
- **Dynamic Portfolio Rotation**: Surgical 1-for-1 replacement using **Conflict-Aware Scanning**. The rotation ranking `inTop3` check filters candidate strikes by type and slices exactly the Top 3 unique strikes. Includes **Atomic Pre-Validation** ensuring replacement candidates pass all safety guards (0.5% scaling) before an exit is authorized. Validates leg swaps against the **Net Premium Swap Cost Check** (must be at least -10 credit/debit), ensuring candidate target quantities are scaled by the $200,000 portfolio cap at 200× leverage first.
...
- **Active Position Trailing Stop Scaling**: In the Paper Trading engine, active positions are dynamically scaled during their trade lifetime. If the trade's unrealized P&L (`grossPnl`) is at or above the trailing threshold `(checkpointAtmPnl * 0.25) + checkpointPnl` (where `checkpointPnl` and `checkpointAtmPnl` act as checkpoints saved at each scaling step, defaulting to `netPremium` and `liveAtmPnl` at start) AND under a hypothetical reduction of the buy leg quantity by **25% of the position's current lot size**, the recalculated position ratio (`sellQty / hypotheticalLotSize`) is at least **1.00 points lower** than the live ATM ratio (meaning `liveAtmRatio >= recalculatedRatio + 1.00`, where the hypothetical lot size must be at or above the dynamic floor of 50% of the position's current lot size (`pos.buyLeg.lotSize * 0.5`)), a partial scale-down exit is triggered. The long leg quantity (`buyLeg.lotSize`) is reduced by **25% of the position's current lot size**, while the short quantity (`sellQty`) remains fully intact. Each reduction is recorded in the `trade_history` table as a **partial exit** record, realizing the exit price P&L for the buy portion closed, while reducing the remaining position's lot size and entry fee, and updating the checkpoint P&L/ATM metrics. The `exit_reason` for the partial exit is recorded in a concise format containing the exact initial and live ATM buy/sell prices, live and recalculated ratios, and original net debit/credit at entry of the position. The position's margin is automatically recalculated and updated in Supabase.
- **Visual Simulation Mode (What-If)**: Instant dashboard simulation for strategy research. Toggle between **Base** and **Extra** credit modes to see recalculated P&L, ratios, and margins across active/closed paper trading positions as well as scanner candidate spreads without affecting the database. When "Extra" is enabled, separate customizable inputs for Call (`C:$`) and Put (`P:$`) extra credits are displayed in call/put theme colors. If the simulated short value exposure exceeds $200,000, the system dynamically scales down both the simulated Buy Qty (lot size) and Sell Qty proportionally to respect the 200X leverage limit, recalculating P&L and margins in real-time.
