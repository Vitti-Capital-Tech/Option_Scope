...
- **Strict Execution-Realistic Paper Trading**: High-fidelity simulation using Ask prices for long legs and Bid prices for short legs. New entries are strictly rejected if real-time, executable quotes (Ask/Bid) are missing from the order book, preventing unrealistic fills from stale model prices.
