...
- **Active Position Trailing Stop Scaling**: In the Paper Trading engine, active positions are dynamically scaled during their trade lifetime. If the trade's unrealized P&L (`grossPnl`) falls below the trailing threshold `(checkpointAtmPnl * 0.25) + checkpointPnl` (where `checkpointPnl` and `checkpointAtmPnl` act as checkpoints saved at each scaling step, defaulting to `netPremium` and `liveAtmPnl` at start) AND under a hypothetical reduction of the buy leg quantity by **25% of its original quantity**, the recalculated position ratio (`sellQty / hypotheticalLotSize`) has a difference of at least `2` points relative to the live ATM ratio (meaning `liveAtmRatio >= recalculatedRatio + 2`, where the hypothetical lot size must be at or above the fixed floor of `0.5`), a partial scale-down exit is triggered. The long leg quantity (`buyLeg.lotSize`) is reduced by **25% of the original long quantity**, while the short quantity (`sellQty`) remains fully intact. Each reduction is recorded in the `trade_history` table as a **partial exit** record, realizing the exit price P&L for the buy portion closed, while reducing the remaining position's lot size and entry fee, and updating the checkpoint P&L/ATM metrics. The position's margin is automatically recalculated and updated in Supabase.
