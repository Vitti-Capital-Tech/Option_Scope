...
- **Scaling & Uniqueness Guards**: Advanced entry filtering including a **0.5% Directional Spot Scaling Guard** (mean-reversion) and a **400-point Strike Diversification** rule (ATM Exit Trading) or **ATM P&L >= $70 and ROI-based Sorting** (Paper Trading) to prevent portfolio concentration and select premium spreads. Strike uniqueness is checked at the database level for both buy and sell legs to block duplicate entries under race conditions.
- **Active Position ATM Ratio-Based Scaling**: In the Paper Trading engine, active positions are dynamically scaled during their trade lifetime in the active positions table. If the live ATM ratio increases by `0.5` or more above its entry baseline (`entryAtmRatio`), its long leg quantity (`buyLeg.lotSize`) is dynamically decreased by `0.15` per `0.5` step (with a maximum decrease cap of `0.5`, allowing the buy quantity to decrease to a minimum of `0.5` of the `originalLotSize`). The position's margin is automatically recalculated and updated in Supabase.
...
- **Hard Portfolio Cap & Displacement**: Maximum 3 active positions per option type (calls/puts) enforced at both the local evaluation level and via a DB-level count guard before every Supabase insert. If the cap of 3 is reached but a new candidate is closer to ATM than the worst active position, and meets the 0.5% spot step and spacing scaling guards, the engine automatically triggers an active-level displacement (exiting the worst position under reason `Lost top 3 and found better target` and entering the superior candidate).
