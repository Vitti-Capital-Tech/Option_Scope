...
- **Visual Simulation Mode (What-If)**: Instant dashboard simulation for strategy research. Toggle between **Base** and **Extra** credit modes to see recalculated P&L and ratios across active positions, history, and KPIs without affecting the database. When "Extra" is enabled, separate customizable inputs for Call (`C:$`) and Put (`P:$`) extra credits are displayed in call/put theme colors. If the simulated short value exposure exceeds $200,000, the system dynamically scales down both the simulated Buy Qty (lot size) and Sell Qty proportionally to respect the 200X leverage limit, recalculating P&L and margins in real-time.
