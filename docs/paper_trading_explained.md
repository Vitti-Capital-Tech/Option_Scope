...
> [!TIP]
> **Long-Only → Long/Short-Pair Replacement (paper AND live):** If an active position is a long-only position (`sellQty = 0`) and candidate scanning finds a candidate long/short spread whose **long OR short strike** matches the active long-only's strike (`buyLeg.strike`), the engine can **exit that long-only** (booked in `trade_history`, `exit_reason: 'Exited for Long/Short Pair'`, `-REP`) and enter the new long/short pair — **if and only if** cap vacancy exists (`count < typeCap`, `combinedCount < combinedCap`) and the candidate's other leg is free (exactly one clash).
> 
> **Only a SHORT-EXIT-origin long-only is replaceable** — one that became long-only when its short leg was bought back, identified by `sellLeg.strike != null` (it retains its old short strike). An **adopted / manually-added** long (`sellLeg.strike === null`, `_adopted`) is **never** replaced. Paper never has adopted longs, so paper behaviour is unchanged.
> 
> **Live execution (promoted from paper).** For an armed-live account the old long-only must be flattened on Delta **before** the new pair opens — the new long is often the SAME symbol (long strike reused), so an un-closed old long would net with it. The engine: cancels the long-only's resting orders + bracket → **reduce-only MARKET close** (`-REPX`) → residual sweep → books `-REP` at Delta's **actual close-fill** (`closingFillPriceFrom`, mark fallback) → deletes the row → then the entry proceeds through the DB guards (which no longer see the deleted row) and `openSpread`. **On close failure the replacement ABORTS** — the old long-only is kept intact and the new pair is skipped for the cycle (retry next), so a pair is never opened on top of an un-flat leg. Paper / dry-run: no real close, books + deletes as before.
> 
> **Zero-Budget Starvation Fix (`effPartMargin`):** When all position slots are occupied (e.g. 4/4 slots full), `remainingBudget` evaluates to `$0`, yielding `partMargin = $0`. To prevent the paper budget check from starving a replacement candidate before it reaches the exit stage, the engine computes **`effPartMargin = freedMargin / remainingSlots`** (adding the replaceable long-only's freed margin into `effectivePoolLeft`), so the candidate passes the budget check and sizes properly.
...
Per-window fields ([scheduled per window](#time-based-filter-schedules), account-level fallback in `paper_trading_config`):
...
Example 1 (Normal Split): Max Combined `4`, Split `70%` → `ceil(0.70 × 4) = 3`. So **max 3 calls and max 3 puts**, but the **combined total never exceeds 4** (e.g. 3C+1P, 2C+2P, …). This is the standard entry cap across paper and live.

Example 2 (All Same Type - Call): Max Combined `4`, All Same Type checked (`Call`) → **4 calls, 0 puts**, with total combined cap 4. `Split %` is disabled in the UI and bypassed by the engine.
...
| Entry governor (paper + live) | `entryGovernor.js` / `paperTradingEngine.js` | **Paper and armed-live**, one shared budget: cross-account, first-come-first-serve top-of-book depth. Accounts sharing one process can't collectively exceed real resting depth (BUY→ask, SELL→bid); a spread is all-or-nothing (both legs or neither) within an 8s window. **Two counters per pool**: real orders draw the real book, simulated ones (paper + dry-run live) draw only a `shadow` copy — so paper contends realistically but can never block a live entry. Blocked accounts get a **website** notification (`entry_block_notifications`, migration `035`) — same 🚦 bell + toast in both modes. See [Cross-Account Entry Governor](#cross-account-entry-governor-paper--live) |
