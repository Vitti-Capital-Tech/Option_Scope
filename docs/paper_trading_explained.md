...
> [!NOTE]
> **Which `daysToExpiry` drives expiry selection** (`expirySelectionMinDte`): on **v1 (live)** it is the account-level `config.daysToExpiry`. On **v2 (paper)** — where the value is per schedule window (migration `019`) — the engine trades **one** expiry at a time, so it rolls to the **nearest expiry that satisfies the smallest (min) days-to-expiry across active windows**. So with windows `1:DTE 1` and `2:DTE 2`, the global expiry follows **DTE 1** (the nearer one) — e.g. today `15` → expiry `16`, **not** `18` — and advances as soon as that nearest-eligible window allows. Each window then guards its own entries with its own value (Guard 2), so a window whose DTE the current expiry doesn't meet simply doesn't enter until the expiry rolls far enough out. The frontend expiry dropdown mirrors this min logic. See [Strategy Versioning](#strategy-versioning-paper-vs-live).
...
| 9 | **Days to Expiry** | `daysToExpiry` (default: 0) | The option expiry date must be at least this many days away from the current time. Options closer to expiry are rejected. **Location depends on `strategy_version`** (migration `019`): on **v1 (live)** it is an account-level Control Panel field; on **v2 (experimental paper)** it moves **per schedule window** (see [Time-Based Filter Schedules](#time-based-filter-schedules)) — each window guards its own entries, while account-global expiry auto-selection uses the **smallest (min)** value across active windows — the engine trades one expiry at a time and rolls it to the nearest expiry the nearest-eligible window allows. See [Strategy Versioning](#strategy-versioning-paper-vs-live). |
...
12. **Days to Expiry** (`daysToExpiry`) — **v2 (experimental paper) accounts only** (migration `019`). The window's value guards its own entries; account-global expiry selection uses the **smallest (min)** across active windows, so the global expiry rolls to the nearest expiry the nearest-eligible window allows. On v1 (live) this stays an account-level Control Panel field and is **not** shown per window. See [Strategy Versioning](#strategy-versioning-paper-vs-live).
...
- **Price** (`hedgeCallPrice` / `hedgePutPrice`) — a **premium budget ($)**. The engine buys the **OTM** strike of that type (call strike > spot, put strike < spot) whose **ask is nearest the target *without exceeding it*** (the highest ask ≤ target = the most protective leg still within budget). If **no** strike quotes at/below the target, the hedge is **skipped** that cycle — it never overpays by falling back to a far-above-target strike (deep-OTM legs near the target are often unquoted, which previously left an expensive strike as the only candidate, e.g. a $3 target filling at $12).
- **Percentage** (`hedgeCallPct` / `hedgePutPct`) — buy qty = **(sum of active short qty of that type, this underlying) × pct/100**.

**Entry** (once, while the window is active and short exposure of the type exists): buys the OTM long whose ask is nearest the budget without exceeding it (skips if none qualify), marks it `buy_leg.isHedge = true`, and records **N = that type's open long positions at entry**. The overlay is **exempt** from the normal portfolio caps, buy-strike-conflict, and exit rules (short buy-back / ladder / ATM-ITM-OTM / expiry) — it has its own exit below.
