...
- **Stale `-PEX` cleanup**: any resting `-PEX` limit found on Delta is a leftover from the older build that rested the partial scale-down (now market-closed, §13.4) — the engine already booked it as sold, so the exchange holds long the book thinks is gone. Each sweep cancels such an order (`cancelStop`) and reduce-only market-closes its remaining size (`PEXCLEAN-<symbol>`), reconciling the exchange to the book. Self-heals to a no-op once none remain.
