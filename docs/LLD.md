...
**Migration index (selected):** `004` verify requests · `009` live_exchange_state · `012` per-window max-debit/exit-type · `016` admin RLS bypass · `036` owner-scoped reads · `037` per-window min-iv & all-same-type (paper) · `018`–`020` strategy_version · `019`/`023` per-window min DTE · `021` trade_days · `022`/`023` hedge leg.
...
- `all_same_type` (BOOLEAN, default `false`, migration `037`, **paper only**) — locks all positions in the window to a single type, bypassing `combined_split_pct`
- `same_type` (TEXT, default `'call'`, check `call` / `put`, migration `037`, **paper only**) — position type (`call` or `put`) when `all_same_type` is enabled; allocates `max_combined_positions` to this type and 0 to the other
- `min_iv_diff` (NUMERIC, default `5`, migration `037`, **paper only**) — per-window minimum IV difference between buy ask IV and sell bid IV (moved from global filters for paper accounts)
