...
| `engine/lib/deltaApi.js` | Backend API adapter for Delta Exchange. Implements WebSockets with auto-reconnect (**exponential backoff + jitter + staggered initial connect**) **plus a stale-feed keepalive watchdog** (terminates a silent/zombie socket after 30s), ticker stream parsing, REST endpoints, and unconfirmed (timestamp = 0) REST ticker backfills. |
| `engine/lib/tickerHub.js` | Shared market-data hub: keeps **one upstream `createTickerStream` per underlying** subscribed to the union of all account engines' symbols, and fans each tick out only to the accounts that want that symbol — so N accounts share 1 Delta WS instead of opening N (avoids the per-IP `429` connection-cap storm). |
...
| `engine/lib/liveExecution.js` | Dry-run-gated live executor wrapping `deltaTradeApi`: `openSpread`, `closeLeg`, `editOrder`, `changePositionBracket`, `placeStop`, `cancelStop`, `positions`, `orders`, `fills`, `recentFillOrderIds`, `orderHistory`, `snapshot`, `walletBalance`, `walletSnapshot` (balance + `available_balance`), `reconcile`. Centralises price sanitisation (`cleanLimitPrice`) and integer contract mapping (`longContracts`/`shortContracts`). |
| `engine/lib/telegram.js` | Fire-and-forget Telegram alerts for armed-real failures (`notifyLiveFailure`, deduped) and trade events (`notifyLiveTrade`, not deduped). Sends are queued **per chat** with a minimum gap and retried on `429` (honouring `retry_after`) / transient errors — Telegram rate-limits per chat, and the engine's minute-boundary bursts were silently losing entry notifications. |
...
- **`reconnectTimer`**: On `onclose` (if `alive` is still `true`), a `setTimeout` schedules a fresh `new WebSocket()`. Any previous timer is always cleared before setting a new one to prevent ghost reconnect loops. **Engine backoff (`engine/lib/deltaApi.js`)**: instead of a flat 3s, the delay is **exponential with jitter** — `min(RECONNECT_MAX_MS 30s, RECONNECT_BASE_MS 3s × 2^attempts) + random(0…base)` — and `reconnectAttempts` resets to 0 the moment a socket actually opens. The very first connect is also **staggered** by a small random delay (`initialJitterMs`, ~0–2s). Rationale: many account streams share one VPS IP; a flat-delay retry made them all re-handshake in lockstep and trip Delta's per-IP `429` rate limit → a perpetual storm. Jitter de-syncs them; backoff throttles repeated failures. The close code/reason and any handshake error (e.g. `429`) are surfaced via `onStatus('disconnected'|'error', info)` so the engine logs *why* per account. *(The scanner UI keeps the flat 3s.)*
...
### Shared Ticker Hub (engine, `engine/lib/tickerHub.js`)

Every account engine runs in one process behind one VPS IP. Each opening its **own** `createTickerStream` meant N identical Delta connections (all subscribing to the same option chain); Delta caps WebSocket connections per IP and rejects the excess with HTTP `429`, so past a threshold the accounts never all connect — a permanent reconnect storm. Backoff/jitter alone can't fix a **concurrency** cap; the connection count itself must drop.

The hub is a module-level singleton that owns **one upstream `createTickerStream` per underlying** and multiplexes every account onto it:
...
Result: 15+ BTC accounts share **1** connection (BTC) instead of 15; ETH accounts get their own hub. Combined with the backoff/jitter above (now protecting just 1–2 connections), the `429` storm is eliminated.
...
|---|---|---|---|
| `paper_trading_accounts` | Supervisor | `id`, `name`, `user_id`, `mode` (`paper`/`live`), `live_enabled`, `paused`, `close_all_requested`, `balance_allocation_pct` (90), `entry_buy_offset` (10), `entry_sell_offset` (3), `default_config` (JSONB), `telegram_chat_id` / `telegram_link_code` (migrations `028`/`029`, per-account Telegram routing) | One row per account. `mode`/`live_enabled`/`paused` gate live execution; switching back to paper forces `live_enabled=false`. `default_config` is the Reset target. `telegram_chat_id` routes that account's live alerts to its own chat (captured via a `/start` deep-link bind); `telegram_link_code` is the pending single-use link token. |
...
| `paper_trading_schedules` | PaperTrading | per-window overrides (see §12) incl. `max_net_premium`, `exit_type`, `exit_points`, `min_days_to_expiry`, `trade_days`, hedge columns, `max_combined_positions`, `combined_split_pct`, and (migration `037`, paper AND live) `min_iv_diff`, `all_same_type`, `same_type` | Permanent, undeletable **Window 1** per account, seeded from base config. |
...
Prefix markers in `log` / `logWarn` / `logError` messages must be **Basic-Multilingual-Plane** characters (codepoint ≤ `U+FFFF`). This deployment's log pipeline mangles 4-byte "astral" emoji on the way to the PM2 files: they either vanish outright (a line logged with 🏃 arrives as `[Acct]  CHASE buy …` — two spaces, no glyph) or decode into an unrelated CJK character. BMP symbols (`⚠ ✖ ✅ ↻ ⌛ …`) survive intact.

The failure mode is silent and costly: `grep` for the emoji returns **0 even when the line is right there**, which is exactly how a working diagnostic gets mistaken for "never fired". All 66 engine log lines were converted; Telegram titles are unaffected and deliberately keep their emoji.

Two rules when adding a log line:

1. Pick a BMP marker, and one not already carrying another meaning (`⚖` is scaling, `✂` is a short exit, `⛔` is a skipped candidate, `⚡` is an already-breached level — reusing one makes two different events indistinguishable).
2. **Never make the marker the only handle.** Keep a distinctive ASCII phrase in the message and grep on *that* — `grep "STALE size data"`, not `grep "⌛"`.
...
| **Positions fallback sync** | 5 minutes | Reloads config + schedules + active positions and runs `reconcileOrphans`, as a safety net against missed Realtime events |
...
* **Full Evaluation (every minute boundary)**: The engine runs `evaluateStrategy(false)` (Full Run) when a new clock-minute crosses (`currentMinute > lastEntryMinute`). In addition to checking exits, it scans for new spread candidates, filters them by ATM P&L >= $50, sorts them to pick the best ROI candidate per buy strike, checks DB-level count/strike restrictions, and inserts new positions into Supabase.

#### Entry Cadence — the minute is claimed on START, not on completion

`lastEntryMinute` holds the minute index (`Date.now() / 60000`) of the last cycle **allowed to open entries**, and it is stamped **when that cycle begins**, immediately after the pre-flight guards pass — not in the `finally` block when it ends.

This distinction is the whole point. The gate previously compared against `lastEvaluated`, a timestamp written on **completion**. A full cycle that overran its minute — a slow chase-fill, a stalled Delta call, a burst of entries — therefore stamped a marker in the minute it had **never actually scanned**, and that minute's entries were silently skipped: no log line, no guard message, just a candidate that qualified at 10:01 and opened at 10:02. A 65s cycle produced entries on **alternate minutes only**. Claiming the minute up front means an overrunning cycle leaves the next minute *unclaimed*, so the first tick after it finishes picks that minute up instead of discarding it.

Three further changes keep cycles inside their minute in the first place:

| Mechanism | Effect |
|---|---|
| **Entry-phase budget** (`ENTRY_PHASE_BUDGET_MS`, default 25s) | Entries are placed serially, each one a chase-fill plus several Supabase guard reads. Once the budget is spent, remaining candidates are **deferred to the next minute's scan** (`⏱ Entry phase budget … spent`) rather than running the cycle into the 60s hang guard. (That guard no longer crashes the process — it fences the stuck cycle per account — but a cycle routinely reaching it still means entries are being placed too slowly.) Checked before each entry, so an in-flight one always completes (never a half-open spread). |
| **Off-lock fills cache** (`LIVE_FILL_POLL_MS` / `LIVE_FILL_STALE_MS`) | `recentFillOrderIds()` used to be **awaited inside the evaluation lock on every ~1s exit tick**, putting a Delta round-trip between the minute boundary and the entry scan for every account holding a position. The fetch now runs off-lock (`kickFillIdsRefresh`) and the loop reads the cached set (`currentFillIds`), never waiting. A never-seeded or stale (>30s) cache returns `null` — the same fail-safe as the old failed fetch, so the exit loop holds rather than mis-inferring a fill. Also cuts Delta egress from ~60 to ~20 calls/min per live account. |
| **Scan gated on `wantEntries`** | `scanTickers` is an O(n²) pass over every strike pair, per type (~24–62 ms per account) and it blocks the shared event loop — every account engine runs in **one process**. Its output feeds the entry path only, so it now runs on entry-eligible cycles only, not on the ~59 exit-only ticks of each minute. |
...
1. **Self-Contained Local Scan**: The headless engine runs its own `scanTickers` (same algorithm as `RatioSpreadScanner`) on calls and puts separately, filtered by option type and ATM direction. Unlike the browser-based version, the headless engine does **not** merge results from the `RatioSpreadScanner` `BroadcastChannel` — it is fully self-contained.
   ...
   **Occupied strikes are removed from the pool BEFORE the scan.** `scanTickers` returns only its top 50 pairs (nearest-ATM first), so feeding it strikes the account already holds spends that budget on candidates the entry guard is guaranteed to reject — and since the same near-ATM strikes keep winning the ranking, an account can **stall outright**. Observed 2026-08-27: with `77200/75000` and `76500/74500` open, every top candidate wanted the **75000** short, so `06:03`→`06:09` produced *no entry at all* while 56% of the budget sat idle across 4 free slots, the same three dead candidates rejected every minute. The pre-filter mirrors `strikesOccupiedBy()` in the entry loop (same expiry + underlying + type; hedge and zero-lot legs don't occupy) but is deliberately limited to **full spreads** (`sellQty > 0`) — a long-only remnant's strike stays in the pool because the entry path can exit and replace one to upgrade it into a full spread.
...
> ⚠️ **Lot rounding is basis-aware (and was silently size-dependent).** All lot quantities in the step math (`deltaBuyQty`, `floorLimit`, `hypotheticalLotSize`, `initialScaledLotSize`) are rounded to **2 dp on the paper basis** (`lotSize` ≈ 1–10 notional units) but to **8 dp for a live position**, which is sized on the contract-value basis (`lotSize = contractValue × contracts`, e.g. `0.001 × 5 = 0.005`). At 2 dp a live step is quantised to `0.01` — so a mid-size position scales at the *wrong* step (50 BTC contracts → 20%/step, not 10%) — and becomes a hard **`0`** once `lotSize < 0.05` (**< 50 BTC / < 5 ETH contracts**). With `deltaBuyQty = 0` every hypothetical lot collapses to `0` → `recalculatedRatio = Infinity` → condition 4 is unsatisfiable and the loop never runs: no slice, no order, **no log line**. The gate is the position's own `buyLeg.contractValue` (`null` → paper), so paper behaviour is unchanged. The slice `trade_id` (`${id}-PE-<lots>`) is stamped at the same precision — at 2 dp a sub-`0.005` live lot collapses all slices onto `-PE-0.00` and the `ignoreDuplicates` upsert drops all but the first. A `deltaBuyQty <= 0` step bails out, and a persisted `initialScaledLotSize` that is not `> 0` is treated as missing and recomputed (older live rows stored a hard `0`). See [live_trading.md](live_trading.md) → *ATM-ratio scale-down*.

> ℹ️ **Step 3's `recalculatedRatio` on the live basis**: the formula shown is the paper form. A live position divides the short **exposure** (`sellQty × sellLeg.contractValue`) by `hypotheticalLotSize` so both sides are on the contract-value basis and the result stays a true contract ratio; paper (`contractValue == null`) uses `sellQty` directly as written.
...
> **SL/TP decoy (migration 031/032).** The level computed above (`computeIndexTriggerLevel`) is the **real** exit — the engine always triggers on it. A **decoy** level is derived from it by shifting `sl_tp_decoy_diff` (per active window; account config is fallback) in the harder-to-trigger direction: `computeDecoyExitLevel(type, real, diff)` = `call: real + diff`, `put: real − diff` (a call exits on spot ≥ level, a put on spot ≤ level, so the shift always makes the decoy fire *after* the real exit). `diff = 0` → decoy == real (backward compatible). On **live** the decoy IS the exchange SL/TP bracket (`openSpread` `longTp`/`shortSl`; moved by `syncExitBrackets` on a window flip and re-armed by `armMissingBrackets`) — a later, engine-down fallback, while the engine's spot-cross catch-all still exits at the real level each cycle. On **paper** it is **observational only** — the real exit is unchanged. For **all accounts** the two levels are stamped on `active_positions.real_exit_level` / `decoy_exit_level` at entry and re-synced on drift (mechanics at the end of this section). *(Un-adopted orphan / protect-only brackets stay at the **real** level — no engine real-level monitor, so the tighter level is safer.)*
...
**Decoy level column sync (all accounts, migration 031/032):** at the top of each cycle (right after `effectiveConfig` is resolved), the engine recomputes `real = computeIndexTriggerLevel(type, buyStrike, effectiveConfig)` and `decoy = computeDecoyExitLevel(type, real, effectiveConfig.slTpDecoyDiff)` for every open position (hedge legs skipped). If either differs from the stamped `pos.realExitLevel` / `pos.decoyExitLevel` (a drift — the active window changed its `exitType`/`exitPoints`/decoy diff), it updates `active_positions.real_exit_level` / `decoy_exit_level` and the in-memory fields; otherwise it's a pure in-memory compare (no DB write). The **entry** path stamps the same two values on insert. This column sync only records the levels — on **paper** they are purely informational; on **live** the actual bracket is moved separately by `syncExitBrackets` (the column just mirrors what that targets). The real exit is never changed here.
...
4. **Gauntlet Filter** (migration 039): a candidate must clear **both** ATM edge floors — `atmPnl >= minAtmPnl` **and** `roi >= minAtmRoi`. Both are **global** (account-level) config and **paper only**; live keeps the historical hardcoded `50` with no ROI floor. `minAtmPnl` (default `50`) is still discounted by the ATM-ratio scaling pct exactly as that constant was; `minAtmRoi` (default `2`) is applied flat, matching the scanner UI's ATM Edge Floors. The dollar floor is scale-dependent — `calculateAtmPnlAndRoi` measures a candidate at `lotSize 1` and only scales down when the $195k short-notional cap binds, which BTC always trips and ETH structurally cannot — so on ETH the ROI floor is the one that carries the decision.
...
4. **Execution**: `entryBuyPrice = spread.ask`, `entrySellPrice = spread.bid`. Entry IVs captured: `entryBuyIv = ticker.askIv`, `entrySellIv = ticker.bidIv`. Baseline ATM ratio (`entryAtmRatio`) and unscaled lot size (`originalLotSize`) are computed.
   - **ATM Ratio Entry Scaling**: If `atmRatioScaling` is enabled, the target ratio is scaled using a percentage offset: `targetRatio = originalRatio + (pct / 100) * (atmRatioVal - originalRatio)`, where `pct` is `atmRatioPctCall`/`atmRatioPctPut` and `atmRatioVal` is the live ATM ratio rounded to 0.25. The entry ratio to use is `ratioToUse = Math.max(spread.sellQty, Math.round(targetRatio / 0.25) * 0.25)`. Both long lot size and short quantity are scaled under the per-underlying leverage limit (BTC 200X / ETH 100X) and the $195k short-notional cap (identical for both) using this `ratioToUse`.
...
### G2. Cross-Account Entry Governor (Paper + Live)

**Files**: `engine/lib/entryGovernor.js` (singleton) · `paperTradingEngine.js` (integration, immediately before `newEntries.push`).

All account engines share **one Node process** (§7 Multi-Account Supervisor). When several accounts pick the same spread in one entry wave, their combined contract size can exceed the real **top-of-book depth** on Delta; independently "filling" all of them is unrealistic in paper and a wasted order round-trip in live. The governor is a module-level singleton that gates entries against a **shared, first-come-first-serve depth budget**. It runs for **both modes against ONE budget**, so a live and a paper account competing for the same book contend with each other — the only arrangement under which the paper result mirrors what live could actually have got.

- **Pools**: `Map<"symbol|side", { remaining, shadow, expiresAt }>`. On first touch in a window a pool is snapshotted from the live feed — `askSize` for a BUY leg, `bidSize` for a SELL leg — and **frozen** for `ENTRY_GOVERNOR_WINDOW_MS` (default `8000`, env-overridable). `null`/non-finite depth → `remaining = Infinity` (unknown never blocks; NB `Number(null) === 0`, so the nullish case is guarded explicitly before coercion). An expired pool re-snapshots fresh depth on the next wave.
- **`reserveSpread(legs, now, { isLive })`** — **a real order is never blocked here.** `isLive` reservations skip the check entirely and only *record* their draw; the gate applies to simulations. Two reasons: live already has better-informed checks downstream (the per-account depth guard at placement time, and `openSpread`'s all-or-nothing chase which unwinds cleanly), and "first come" is decided by timer **registration order**, which is fixed for the life of the process — so at 3 live accounts the same one wins every minute, and at 40 the last-registered ~37 would never trade a contended strike at all. A gate whose fairness degrades with account count cannot sit in front of real money.
  Simulated callers still go two-pass, all-or-nothing: **Pass 1** checks `shadow >= qty` for **all** legs and on a shortfall returns `{ ok:false, blockedLeg:{symbol,side,qty,available,capacity} }` **without consuming anything** (`capacity` = the top-of-book snapshot the 8s window opened with, so `available === capacity` means **nobody drew** — the book was simply too thin — while `available < capacity` is real contention); **Pass 2** consumes. Consumption is asymmetric — `shadow` is decremented by everyone, `remaining` only by real orders (now purely observational, recording what live took so paper's contention stays realistic). Single-threaded JS makes the body atomic; no locks.
> [!IMPORTANT]
...
- **Contract qty** — the two modes differ *only* here:
  - **LIVE** (`liveArmed`): reuses the counts the sizing block already decided — `longC = round(adjustedLotSize / liveLongCV)` and `shortC = adjustedSellQty` (which *is* the short contract count on the live basis). Deliberately **not** re-derived, so the governor reserves exactly what the order will request and can never drift from it.
  - **PAPER**: mirrors that live sizing math — `scale = partMargin / baseMargin`, `longC = round(scale)`, `shortC = round(longC × ratioToUse)`, clamped by the `$195k` short-notional cap via `contractValue` — because paper has no contract count of its own. Used **only** for the depth check; the paper position's fractional-lot P&L sizing is unchanged. Missing `contractValue`/`partMargin` → governor is a **no-op** (entry allowed rather than a manufactured block).
- **On block**: `continue` — the candidate is never pushed to `newEntries`, so nothing persists and the account stays flat for this spread — plus a fire-and-forget INSERT into `entry_block_notifications` (migration `035`) driving the **website** toast + 🚦 panel. Not Telegram. Identical row shape for both modes; `details.mode` records which one, purely for log attribution.
- **Scope**: runs for paper (`mode !== 'live'`) and for **armed live** (`liveArmed = mode === 'live' && live_enabled`). `liveArmed` does **not** depend on `DELTA_LIVE_DRYRUN`, so a **dry-run live** account is governed too — you can validate the whole path before arming. Note the `isLive` flag is a *stricter* predicate than `liveArmed`: it is `mode === 'live' && live_enabled && !dryRun`, so dry-run draws the shadow only. A live-mode account with `live_enabled = false` is skipped: it is sized through the unarmed notional-cap branch with no `contractValue`, so there is no reliable contract count to reserve, and it places no real orders.
- **Composition with the live depth guard**: live *also* keeps its per-account thin-book guard at execute time (`longC > askSz` → skip). The governor runs first, at candidate time, and is the stricter of the two (for the first account in a window the pool equals `askSize`, so governor-pass implies guard-pass). The guard remains the backstop for depth that thinned between the two checks.
  > **Known interaction:** the two checks read the feed at different moments and the governor freezes its pool for the window. If the governor reserves and the execute-time guard then skips the entry, that reservation is not released and stays consumed for the rest of the window (≤ `ENTRY_GOVERNOR_WINDOW_MS`, default 8s) — conservative, self-healing on the next wave, and it can only cause an *extra* block, never an over-fill.
...
- **Ratio Spread Scanner Simulation**: Driven directly by the configuration-level ATM Ratio Entry settings (`atmRatioScaling` toggle and `atmRatioPctCall` / `atmRatioPctPut` offsets). When enabled:
  - The **scaled short quantity (`scaledSellQty`) is computed inside `scanTickers`** (before the Max Debit filter); `ResultTable.jsx` **consumes** it and renders the derived margins, net premiums, and projected ATM P&Ls in real-time under the per-underlying leverage limit (BTC 200X / ETH 100X) and the $195k portfolio cap — it no longer recomputes the scaling itself.
...
- **Margin Calculation**: Derived from **spread entry prices** (not ATM chain data), so it is always available:
  ...
  `leverage = leverageFor(underlying)` — **ETH `100`, everything else `200`**
  `margin = (buyPrice × buyLotSize) + (shortValue / leverage)`

  The leverage is **per underlying**; the $195,000 short-notional cap is **not** — it is the same for both. So the identical short notional costs an ETH account twice the margin a BTC account pays, and an ETH position sized against a fixed margin budget comes out roughly half the notional. `leverageFor()` lives once in `engine/lib/utils.js` and once in `src/scannerUtils.js` (the browser twin, used by `ResultTable`'s displayed margin and `PaperTrading`'s position margin); the two must move together, because the scanner's margin/ROI is exactly the number a user checks an engine decision against. It accepts either the underlying code (`'ETH'`) or a full Delta symbol (`'C-ETH-2800-030926'`), since the display path only has the symbol to hand.
...
- `max_combined_positions` (INTEGER, default `4`, migration `027`, promoted to live) — hard cap on total open full spreads (calls + puts); also the per-position-margin divisor
- `combined_split_pct` (NUMERIC, default `70`, migration `027`, promoted to live) — derives the per-type cap `ceil(split% × combined)` for both calls and puts
- `all_same_type` (BOOLEAN, default `false`, migration `037`, promoted to live) — locks all positions in the window to a single type, bypassing `combined_split_pct`
- `same_type` (TEXT, default `'call'`, check `call` / `put`, migration `037`, promoted to live) — position type (`call` or `put`) when `all_same_type` is enabled; allocates `max_combined_positions` to this type and 0 to the other
- `min_iv_diff` (NUMERIC, default `5`, migration `037`, promoted to live) — per-window minimum IV difference between buy ask IV and sell bid IV (moved from global filters into schedule windows for all accounts)
...
- **`effectiveConfig` Generation**:
  - If a schedule window matches, the engine creates an `effectiveConfig` by spreading the account's base configuration and overriding scheduled properties: `maxCombinedPositions`, `combinedSplitPct`, `allSameType`, `sameType`, `minIvDiff` (migration `037`), `minLongDist`, `minStrikeDiff`, the ATM-ratio scaling fields, `spotDiff`, `maxNetPremium`, `exitType`, `exitPoints`, `slTpDecoyDiff`, `shortExitPrice`, `variableExitSlices`, `longExitSlices`, `minDaysToExpiry` (v2), and the hedge fields.
  - If no schedule window matches, `effectiveConfig` falls back to the base account config.
  - All scanner candidate matching, position-limit evaluations, candidate ranking pools, and the **exit-type check** (paper) / spot-cross catch-all (live) inside `evaluateStrategy()` utilize this `effectiveConfig`. When `allSameType` is active, `derivedTypeCap(effectiveConfig, type)` gives full capacity to the chosen type and 0 to the opposite type across both paper and live.
...
- **Full-position flattens use reduce-only MARKET closes** (`live.closeSymbol`, IOC — immediate fill), **not** resting/marketable limits — these are risk / user-driven exits where getting flat *now* beats price protection (a marketable limit that rests even ~1s can slip **$10–15** on a moving option, and the flatten deletes the tracked row right after, so a resting limit would orphan the leg): (1) the engine's **spot-cross / expiry catch-all Full Exit** (cancel resting → market-close each leg `${id}-CAXS`/`-CAXB`/`-HX` → residual `CASWEEP` sweep → book one `${id}` full exit), and (2) the **manual exit** (`${id}-MXS`/`-MXB`/`-MXH` → `MXSWEEP`). The `exitLong`/`exitShort` bid/ask touch prices are kept only to **book** the P&L, not sent as order prices. The resting short buy-back (`-SEX`) and long ladder (`-LE`) stay **limits** (profit scale-outs that wait for a favourable price). At **expiry** no order is sent — Delta cash-settles at intrinsic.

### 13.5 Exchange brackets (engine-down risk backstop)

- `openSpread` attaches a **spot-triggered bracket** per leg at the **decoy** exit-type level (`computeDecoyExitLevel(computeIndexTriggerLevel(...), slTpDecoyDiff)` — migration 031; ATM = buy strike, ITM/OTM = ± points, then shifted `slTpDecoyDiff` pts harder-to-trigger; `diff = 0` → real level): long → `bracket_take_profit_price`, short → `bracket_stop_loss_price`, `bracket_stop_trigger_method: spot_price`. Delta auto-cancels it when the leg closes. The engine's own spot-cross exit fires at the **real** level first each cycle.
...
- Live accounts size from the **live Delta USDT wallet balance** (not the paper `$195k`). `part = remaining ÷ free slots`, remaining = `balance × balance_allocation_pct − usedMargin`, free slots from the **active window's** `maxCombinedPositions` (migration `027` combined model, promoted to live). The remaining budget is capped at Delta's `available_balance` minus the allocation reserve (no safety factor: all free margin is used), and each candidate is fitted to the Delta free margin still unclaimed that cycle (1 unit doesn't fit → skip). A Delta `insufficient_margin`/`insufficient_commission` rejection retries once with the order re-sized to Delta's fresh available balance; 2 in a row → new live entries pause 10 min. Unit margin via `calcMargin` using real per-contract `contractValue` + current spot; `scale = part ÷ unitMargin` (floored at 1); `longC = round(scale)`, `shortC = round(longC × ratioToUse)`. Then capped so short notional (`spot × shortC × contractValue`) ≤ **$195,000**. Missing `contractValue` for either leg ⇒ **skip the entry** (never guess). Logs `💰 LIVE size…` / `🧢 LIVE qty capped…`.
...
- **Chase-fill (all-or-nothing)**: `openSpread`/`submitChase` chase each leg to a full fill (`editOrder` re-price, up to `ENTRY_CHASE_ATTEMPTS`=2 every `ENTRY_CHASE_POLL_MS`=1200). The cross per attempt is `max(ENTRY_CHASE_BUMP`=1 `$, premium × ENTRY_CHASE_BUMP_PCT`=3 `%)`, clamped to 50% of premium — a flat-only cross is 0.5% of a $200 leg and cannot reach a moving book (see [live_trading.md](./live_trading.md#entry-chase-fill)). An abort on an **unfilled** leg queues a bounded same-minute retry (`ENTRY_RETRY_DELAY_MS`=10000, `ENTRY_RETRY_MAX_PER_MIN`=2); a submit **rejection** does not; if still unfilled → **unwind + abort**, no insert. The chase **sleeps inside the evaluation lock**, so `attempts × pollMs × legs` lands directly on the cycle that must also open the next minute's entries: the former 3×5000 cost ~15s per leg → ~30s per spread, and two spreads in one cycle overran the 60s hang guard. 2×1200 costs ~2.5s per leg (~5s per spread) and still gives a widening quote two chances before the unwind. Bounded overall by `ENTRY_PHASE_BUDGET_MS` — see [Entry Cadence](#entry-cadence--the-minute-is-claimed-on-start-not-on-completion).
- **Delta HTTP deadlines**: `fetch` has no default request timeout, so a half-open socket used to hang a call until the OS TCP timeout — minutes, with that account's exits *and* its next entry scan frozen behind the evaluation lock. Every Delta call now carries an `AbortSignal.timeout`: signed reads `DELTA_READ_TIMEOUT_MS`=6000, signed **writes** `DELTA_WRITE_TIMEOUT_MS`=15000, public reads `DELTA_PUBLIC_TIMEOUT_MS`=10000. Writes get the long leash deliberately — aborting the client side of an order Delta already accepted leaves an **orphan on the exchange**. Aborts are re-labelled (`Delta API POST /v2/orders timed out after 15000ms`) so callers log the real cause instead of a bare `AbortError`.
- **Top-of-book depth guard** (armed-live, pre-`openSpread`): enter only if each leg's qty is **`< ` the resting top-of-book size** on the side it hits — long/hedge (buy) vs `askSize`, short (sell) vs `bidSize` (Delta `quotes.bid_size`/`ask_size`, captured as `bidSize`/`askSize`). `qty >= depth` on any leg → **skip the whole entry** (avoids uneven/lopsided fills). Fail-open when a size is unknown (chase-fill is the backstop).
- **Same-product collision guard** (Fix B): skip any candidate whose leg symbol already holds a position (`live.positions()` ∪ tracked book). **Dangling short** (Fix A, revised 2026-09-24): `sellQty>0`, short open, long gone (zeroed on purpose or confirmed missing) → **admin alert only**, no close. **Benign `no_position_for_reduce_only`** (Fix D): treated as `{ok:true, alreadyClosed:true}`.
- **Atomic-ish insert**: orders placed first, row written last; a failed insert unwinds via `${id}-ORPHX`; the in-memory book is rebuilt only from `persistedEntryIds`. **Residual sweep** (`sweepResidualLegs`) reduce-only market-closes any contracts left open before a full-exit row is deleted — on the laddered exit (`-LSWEEP`, long leg) and the spot-cross/expiry catch-all (`-CASWEEP`, all legs; side derived from the live-size sign).
...
- `reconcileOrphans` handles **book→exchange** (tracked row gone from Delta → book + remove). `protectOrphanExchangePositions` handles **exchange→book**: a **long orphan** (known symbol, buy strike free) → `adoptLongOrphan` (build from Delta's real `entry_price`, insert first, **arm a TP bracket only — NO scale-out ladder**; a manually-added / standalone long has no short to justify a ladder, so it's managed by the TP bracket + the ATM/ITM/OTM spot-cross and expiry catch-alls. The laddered exit is reserved for the pair short-exit flow — `handleLiveRestingExit` / `externalShortExitToLongLadder`); an **untracked short** → **admin alert only, never closed**; an untracked long while an untracked short is open → alert only (not adopted); can't adopt → arm a TP bracket + alert (breached level → alert only, no close). A tracked leg counts as gone only after 3 consecutive successful snapshots + 90 s. A failed positions fetch is retried until Delta answers, and if every tracked leg vanishes at once, Delta is re-queried every 10 s until the legs return or the absence is confirmed. Before booking (and cancelling brackets), Delta order history must also show closing-side fills since the leg was last seen open, adding up to at least the tracked contracts; otherwise the position is held with SL/TP intact and the admin is alerted (see live_trading.md, *Absence confirmation*).
...
- **Manual changes on Delta**: full close → book + delete (only after the legs are confirmed missing and order history shows the closing fills); **short-only close** → `externalShortExitToLongLadder` (dangling-long recovery: `sellQty>0`, short 0, long>0, age>90s, `_shortEverOpen`, short confirmed missing + buy-back found in order history; books `${id}-SE`, converts to long-only, places `${id}-LE-*`, latched); **TP/SL change** → `adoptManualBrackets` (into `brkLevel` only); **bracket cancel** → `armMissingBrackets`; **partial reduction** → `reconcilePartialReductions` (book `${id}-XPR-L|S-<remaining>`). Guards: armed-real only, reuses the sweep's snapshot, 120s grace (`orphanSeenAt`), once-per-orphan latch.
...
`engine/lib/telegram.js` (`notifyLiveFailure`), fire-and-forget, armed-real only. Env: `TELEGRAM_BOT_TOKEN` (required), `TELEGRAM_CHAT_ID` (default destination), `TELEGRAM_DEDUPE_MS` (60000). Triggers: order/close/stop/bracket failure (benign codes suppressed), protective close failure, entry aborted (incl. margin re-size / pause note), orphan reconcile failure, armed-but-no-credentials. **Admin alerts** (`notifyAdmin`, sent to the account's chat **and** the global `TELEGRAM_CHAT_ID`) cover every case where the engine deliberately does not act: untracked short / untracked long beside it, short open without its long, all tracked legs missing from Delta's list, and a missing position with no closing fill in order history. Each message: account, failure, Delta error, UTC timestamp; deduped.

**Delivery** (shared by failures and trade events): sends are queued **per destination chat**, serialised with a `TELEGRAM_MIN_GAP_MS` (1200) gap, and additionally held behind a **bot-wide** floor `TELEGRAM_GLOBAL_GAP_MS` (150) — Telegram caps both per chat (~20/min for a group) and per bot (~30/s), and the engine's minute-boundary bursts go to *different* chats, so only the global ceiling explains the observed `429`s. Retried up to `TELEGRAM_MAX_RETRIES` (3): a `429` waits Telegram's own `retry_after` (capped 30 s) and also nudges the global floor (the response does not say which limit was hit), `5xx`/network back off ~2 s, a non-429 `4xx` is permanent and never retried. Queue bounded at `TELEGRAM_MAX_QUEUE` (200) per chat; retry timers are unref'd so a restart is never held open. A rolling 60 s record of send attempts backs a **volume** suffix on every 429 / give-up log line (`volume: 3 to this chat, 27 bot-wide, in the last 60s`) — successful sends were previously unlogged, so a 429 could not be attributed to either ceiling. See [Delivery](./live_trading.md#delivery-per-chat-queue--retry) for the 2026-09-08 incident.
...
- **Max Margin Utilised %** (SchedulePanel): `(peak FULL-spread margin / allocated balance) × 100` — the peak share of the allocated balance consumed by concurrent **full-spread** positions at any single instant during the session for that window. Computed by an event sweep (enter `+margin` / exit `−margin`, peak of the running sum) over **position lifetimes**, using each position's **actual stored margin** (`pos.margin` — the same value the engine subtracts as `usedMargin`; the even `allocBal/cap` slot split is only a fallback when a row has no margin), NOT a slot approximation.
  - **Lifetimes** cover the whole session day, so the peak reflects the moment the most margin was deployed — not just what's open right now:
    - Open **full spreads** (`sellQty > 0`) → `[entry, now]` at their current (full) margin.
    - Every other position (open-but-long-only **or** already closed) → its **full-spread phase** `[entry, short-close]` at the full margin, rebuilt from `trade_history` (widest `[entry, exit]` span, MAX margin) — deduped against still-open full spreads so nothing is double-counted.
  - **Long-only phase excluded:** a position holds full margin only while both legs are on. Only `trade_history` rows with **`sell_qty > 0`** (short still present) are used, so the long-only ladder/wind-down exits (`-LE-*`, expiry-long — all `sell_qty = 0`) are dropped; the interval ends at the short close. Allocated balance = live wallet × alloc% (live) or `(initialBalance + realized P&L) × alloc%` (paper). Hover tooltip shows exact peak margin ($) vs allocated balance ($).
