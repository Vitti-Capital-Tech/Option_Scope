...
| Mechanism | Effect |
|---|---|
| **Entry-phase budget** (`ENTRY_PHASE_BUDGET_MS`, default 25s) | Entries are placed serially, each one a chase-fill plus several Supabase guard reads. Once the budget is spent, remaining candidates are **deferred to the next minute's scan** (`⏱ Entry phase budget … spent`) rather than running the cycle into the 60s hang guard — which crashes the process and takes *every* account's entries down with it. Checked before each entry, so an in-flight one always completes (never a half-open spread). |
...
- **`reserveSpread(legs, now, { isLive })`** — two-pass, all-or-nothing. **Pass 1** resolves every leg's pool and checks the caller's counter (`isLive` → `remaining`, else `shadow`) `>= qty` for **all** legs; if any is short it returns `{ ok:false, blockedLeg:{symbol,side,qty,available} }` **without consuming anything** (freed depth stays for the next account). **Pass 2** (all fit) decrements `shadow` always and `remaining` only when `isLive`, then returns `{ ok:true }`. Single-threaded JS makes the body atomic → "first come" is the first caller this window; no locks.
> [!IMPORTANT]
