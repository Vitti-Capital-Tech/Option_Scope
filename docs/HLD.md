...
3. **Scaling & Uniqueness Guards**: 
   ...
   - **Active Position ATM Ratio Scaling**: Evaluates active positions inside the exit loop. If the live ATM ratio (`roundedAtmRatio` of ATM bid / OTM ask) increases by `0.5` or more relative to its entry baseline (`entryAtmRatio`), the long leg quantity (`buyLeg.lotSize`) is decreased by `0.15` per `0.5` step (up to a maximum reduction of `0.5`, capping the buy quantity multiplier at a minimum of `0.5` of the `originalLotSize`). This is a **one-way ratchet adjustment**; the quantity can only decrease and will never increase back even if the live ATM ratio decreases. When updated, the position's margin is recalculated using `calcMargin` and saved back to the database.
