...
- **Themed Modals**: Features custom React modals for creating, editing, and deleting accounts. The Create Account Modal includes options to set up the default strategy filters (pre-filled with the current page's active filters), allowing users to define strategy thresholds immediately upon creation. It uses `react-hook-form` to validate inputs (checking for empty names and non-positive initial balances/edited balances). Styled with active theme CSS variables (`--bg2`, `--bg3`, `--text`, `--text-dim`) with inline loading spinner SVGs during DB operations, and warns the user if they try to delete an account with active positions.
...
- **Single Account Engine**: Each loop executes the trading strategy independently, scoped by its specific account ID. Reuses scanner-style candidate selection to simulate positions. Filters candidates by projected ATM P&L >= $50 and sorts them by ROI descending. Enforces account-specific position limits (max 3 positions per option type), independent balance/margin checks (margin cap against account balance), IV tracking, and Supabase synchronization. If no config row exists for a new account, the engine **auto-creates** a default config row with standard values (BTC, 800 strike diff, etc.).
...
2. Evaluate active positions for rotation or ATM/expiry exit triggers every second to prevent slippage, while scanning and entering new positions on the 1-minute boundary. Positions are refreshed from the database every **30 seconds** as a fallback safety net in case Realtime events are missed.
...
10. Open new positions up to 3 per type from the ranked candidate list. Each entry must pass an **account margin cap** check (total deployed margin + candidate margin ≤ account balance) in addition to the DB count guard that prevents exceeding 3 even under race conditions.
