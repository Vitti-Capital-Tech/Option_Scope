...
3. **Scaling & Uniqueness Guards**: 
   ...
   - **Active Position ATM Ratio Scaling**: Evaluates active positions inside the exit loop. If the live ATM ratio (`roundedAtmRatio` of ATM bid / OTM ask) increases by `0.5` or more relative to its entry baseline (`entryAtmRatio`), the long leg quantity (`buyLeg.lotSize`) is decreased by `0.15` per `0.5` step (up to a maximum reduction of `0.5` of the baseline lot size). The scaled quantity is capped at an **absolute minimum floor of 0.5** (or the original lot size if the original was already below 0.5). Each reduction is recorded in the `trade_history` table as a **partial exit** record with `sell_qty = 0`, realizing the proportional entry fee and current long leg bid price P&L, while reducing the remaining position's lot size and entry fee proportionally while the active position's `sellQty` remains fully intact. This is a **one-way ratchet adjustment**; the quantity can only decrease and will never increase back even if the live ATM ratio decreases. When updated, the position's margin is recalculated using `calcMargin` and saved back to the database.
