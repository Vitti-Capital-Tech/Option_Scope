...
3. **Scaling & Uniqueness Guards**: 
   ...
   - **Active Position Dynamic Scaling**: Evaluates active positions inside the exit loop. Scaling triggers when three conditions are met: (1) the position is **profitable** (`currentGrossPnl > 0`), preventing false triggers at entry when PnL is zero, (2) the PnL has **deteriorated below the trailing threshold** (`currentGrossPnl <= checkpointAtmPnl * 0.25 + checkpointPnl`), and (3) the live ATM ratio (in 1:x format, `buyIntrinsic / sellIntrinsic`) has **increased by at least 2** from the position's own current ratio (`pos.sellQty / currentLotSize`), meaning `liveAtmRatio >= (pos.sellQty / currentLotSize) + 2`. Only the long leg quantity (`buyLeg.lotSize`) is reduced by `0.25` (down to an absolute minimum floor of `0.5`), while the short quantity (`sellQty`) remains fully intact. After each step, `maxAtmRatio` in metadata is updated to reflect the new ratio of the position, and checkpoint values are saved. `entryAtmRatio` is never modified (it is a historical value). Each reduction is recorded in the `trade_history` table as a **partial exit** record, realizing the proportional entry fee and exit price P&L for the buy portion closed. The remaining position's margin is recalculated using `calcMargin` and saved to the database. This scaling can recur in steps of `0.25` lot size down to the `0.5` floor limit.
