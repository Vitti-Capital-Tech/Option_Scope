...
- **Multi-Account Paper Trading Supervisor (Node.js)**: The headless process (`paperTradingEngine.js`) runs a supervisor manager that queries `paper_trading_accounts` and spawns isolated, **parallel** engine execution loops (`startSingleAccountEngine`) for each account using `Promise.allSettled`. This reduces startup time from ~30s (sequential) to ~3s regardless of account count. The supervisor listens to Supabase database events (INSERT, UPDATE, DELETE) on the `paper_trading_accounts` table to dynamically start, stop, or hot-reload single account engine threads in real-time without process restarts.
...
3. **Scaling & Uniqueness Guards**: 
   ...
   - **Days to Expiry Filter**: Prevents entry on expiries with fewer days remaining than the configured `days_to_expiry` value (default 0). Auto-selects the nearest expiry satisfying this threshold.
...
9. **Dynamic Portfolio Rotation**:
   The engine compares existing positions against current top scanner results:
     - **Displacement Check**: If a position is no longer in the top protected strikes (the protection check filters candidates by type and slices them dynamically based on the configured maximum calls/puts active positions) AND a superior candidate (closer to ATM) is available, it is marked for rotation.
     - **Leg Swap Logic**: If a superior candidate has the exact same sell strike, the engine executes a Leg Swap — exits only the long leg (recording buy-leg-only PnL in `trade_history`) and updates the position row in-place with the new buy leg. If the sell quantity changes, the sell entry price is **weighted-averaged**. It validates the swap against the **Net Premium Swap Cost Check** (`netPremiumSwap = (deltaQty * latestSell) - (s.buyPrice - latestBuy) >= config.legSwapNetPremium`, where `deltaQty = getScaledSellQty(s) - pos.sellQty` scales the target quantity first by the $195,000 portfolio cap at 200× leverage), which rejects any swaps where netPremiumSwap < config.legSwapNetPremium.
    ...
    - **Worst-First Processing**: Active positions are evaluated from farthest-to-ATM first, ensuring the least desirable legs are rotated out first.
     - **Cycle Guards**: Rotation only begins once the portfolio hits a threshold (e.g. matching configured portfolio limit) and is capped dynamically per evaluation cycle based on the configured maximum calls/puts.
...
13. **Time-Based Filter Schedules**: Supports configuring multiple time windows per account. Frontend configuration inputs, the visual timeline, database storage (`TIME` type), and engine evaluation matches operate natively in Indian Standard Time (IST) for user convenience. The timeline bar displays the daily trading cycle starting and ending at `05:30` IST (corresponding to `00:00` UTC Delta Exchange daily rollover/day boundary), ensuring empty slots display at the end of the bar. When a schedule window matches, the engine overrides specific parameters (`numberOfCalls`, `numberOfPuts`, `minStrikeDiff`, `minLongDist`) with the window's scheduled values, falling back to the base account configuration when no window matches. Changes made in the UI are automatically synced via a debounced auto-save (showing live sync indicators: `✓ Live Synced`, `Syncing...`, `Overlap Detected`), and the engine reloads updates in real-time via Supabase Postgres Realtime subscriptions.
