...
- **Stale-Feed Watchdogs (engine)**: A dropped socket fires a `close` event and reconnects — but a *half-open* socket can keep looking connected while the server silently stops sending data (observed: ~4 min of missed entries). Two guards recover proactively: a **stream-level keepalive** terminates a fully-silent socket after 30s, and an **engine-level per-tick option-feed watchdog** forces a reconnect (within ~30s) when option quotes go stale while spot is still live (perp and options share one connection, so an options-only stall is otherwise invisible to the spot-staleness check).
...
3. **Scaling & Uniqueness Guards**: 
   ...
   - **Paper Balance & Combined-Position Sizing (paper only, migration `027`)**: Paper accounts model a **funded balance** — dynamic equity = `initial_balance` + cumulative realized P&L. A **Balance Allocation %** carves the tradeable margin pool (the rest is buffer), and each schedule window has a **Max Combined Positions** cap (calls + puts) plus a **Split %** that derives the per-type cap `ceil(split% × combined)`. Per-position margin = allocated pool ÷ the **active window's** Max Combined, computed on the *remaining* pool so each new time window re-divides the leftover balance; positions scale to fill that margin and self-skip when the pool is exhausted. The **Max Combined + Split% entry caps now apply to live accounts too** (migration `027` combined model, promoted to live; `numberOfCalls`/`numberOfPuts` dropped in migration `034`); live still sizes on the real Delta wallet balance ÷ Max Combined rather than a funded equity.
   - **Daily full-deployment fill (configurable time, paper AND live — migration `030`, promoted to live)**: normal sizing reserves budget for every empty slot, so idle balance can accumulate when candidates are scarce. Once per day, on the upward crossing of the configured IST time (default `04:30`, latched per IST date), the engine runs a one-time fill that **concentrates** the remaining pool across only the spreads scanning actually finds openable (÷ openable count, not ÷ empty slots) — respecting the Max Combined cap and the normal entry filters (no forced/low-quality fills; if nothing qualifies, nothing opens). Live concentrates the real wallet allocation, paper its equity; each position stays bounded by the $195K short-notional cap. Existing positions are untouched.
...
10. Open new positions up to the configured limit per type from the ranked candidate list, enforced by local and DB-level count guards (the cap counts full spreads only — `sell_qty > 0` — so held long-only rows are excluded). **Paper Trading Long-Only Replacement:** In paper trading, if candidate scanning finds a long/short pair whose short strike matches an active long-only position (`sell_qty = 0`), the engine exits the long-only position (booking `trade_history` with `exit_reason: 'Exited for Long/Short Pair'`) and opens the new long/short pair with a new long strike if position cap vacancy exists.
