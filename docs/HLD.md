...
4. Project spread values to the ATM boundary using live option chain shifting:
   - **At ATM Ask/Bid**: Pulls the current option chain Bid for the ATM strike (long leg) and the Ask for the OTM strike at `ATM ± strikeDiff` (short leg). If the exact strike is missing from `tickerData`, the nearest available strike within `max(10% of spot, 5000)` is used as a fallback. Directly shows the ATM premium ratio below the prices, rounded to the nearest 0.25. Displays `—` when no suitable ticker exists.
...
3. **Scaling & Uniqueness Guards**: 
   ...
   - **Buy Strike Uniqueness**: Ensures new buy strikes are unique in the database for the same underlying and type via a DB-level exact-match check (`buyConflict`), preventing duplicate entries under race conditions.
    - **Active Position ATM Ratio Scaling**: Evaluates active positions inside the exit loop. If the live ATM ratio (`roundedAtmRatio` of ATM bid / OTM ask) increases by `1.0` or more relative to its entry baseline (`entryAtmRatio`), and exceeds the trade's lifetime maximum ATM ratio (`maxAtmRatio`), the long leg quantity (`buyLeg.lotSize`) is decreased by `0.1` per `0.5` step (up to a maximum reduction of `0.5` of the baseline lot size). The scaled quantity is capped at an **absolute minimum floor of 0.5** (or the original lot size if the original was already below 0.5). Each reduction is recorded in the `trade_history` table as a **partial exit** record with `sell_qty = 0`, realizing the proportional entry fee and current long leg bid price P&L, while reducing the remaining position's lot size and entry fee proportionally while the active position's `sellQty` remains fully intact. The position's `maxAtmRatio` is updated to the `liveAtmRatio` to prevent redundant scaling down under subsequent fluctuations. This is a **one-way ratchet adjustment**; the quantity can only decrease and will never increase back even if the live ATM ratio decreases. When updated, the position's margin is recalculated using `calcMargin` and saved back to the database.
