...
6. **Visual Simulation Mode**: A "What-If" dashboard layer that allows users to simulate the impact of adding custom premium/credit to their strategy visually (including P&L, ratio, and margin recalculations) without affecting the database. Toggling "Extra" credit mode displays separate inputs for Call (`C:$`) and Put (`P:$`) extra credits. If the simulated short value exposure exceeds $200,000, both the simulated Buy Qty (lot size) and Sell Qty are scaled down proportionally to respect the 200X leverage limit, dynamically recalculating the positions' P&L and margins.
